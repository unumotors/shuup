...
var Picotable = (function (m, storage) {
    ...
    var cx = function generateClassName(classSet) {
        ...
        Util.map(classSet || {}, function (flag, key) {
            var className = null;
            if (key === "") {
                // The empty string as a class set means the entire value is used if not empty
    ...
    function paginator(paginationData, setPage) {
        ...
        var pageLinks = [];
        var pageLink = function pageLink(page, title) {
    ...
    function buildColumnChoiceFilter(ctrl, col, value) {
        var setFilterValueFromSelect = function setFilterValueFromSelect() {
            var valueJS = JSON.parse(this.value);
            ctrl.setFilterValue(col.id, valueJS);
        };
        var select2Config = function select2Config() {
    ...
    function buildColumnRangeFilter(ctrl, col, value) {
        var setFilterValueFromInput = function setFilterValueFromInput(which) {
            var filterObj = Util.extend({}, ctrl.getFilterValue(col.id) || {}); // Copy current filter object
        ...
        return m("div.range-filter", [m("div.input-wrapper.min", { key: "min" }, minInput), m("div.input-wrapper.max", { key: "max" }, maxInput)]);
    }

    function buildColumnTextFilter(ctrl, col, value) {
        var setFilterValueFromInput = function setFilterValueFromInput() {
    ...
    function renderTable(ctrl) {
        var data = ctrl.vm.data();
        if (data === null) {
            // Not loaded, don't return anything
            return;
        }
        ...
        var columnFilterCells = Util.any(data.columns, Util.property("filter")) ? Util.map(data.columns, function (col, columnNumber) {
            return buildColumnFilterCell(ctrl, col, columnNumber);
        }) : null;
        var thead = m("thead", [m("tr.headers", columnHeaderCells), columnFilterCells ? m("tr.filters", columnFilterCells) : null]);
    ...
    function getMobileFilterModal(ctrl) {
        ...
        var filters = Util.map(data.columns, function (col) {
            ...
            return m("div.single-filter", m("h3", col.title), buildColumnFilter(ctrl, col));
        });
        return m("div.mobile-filters.shuup-modal-bg", { key: "mobileFilterModal" }, [m("div.shuup-modal-container", [m("div.shuup-modal-header", [m("h2.pull-left", [m("i.fa.fa-filter")], gettext("Filters")), m("button.btn.btn-success.pull-right", {
            onclick: function onclick() {
                ctrl.vm.showMobileFilterSettings(false);
            }
        }, "Done"), m("button.btn.btn-gray.btn-inverse.pull-right", {
            onclick: ctrl.resetFilters,
            disabled: Util.isEmpty(ctrl.vm.filterValues())
        }, lang.RESET)]), m("div.mobile-filters-content", filters)])]);
    }
    ...
    function renderMobileTable(ctrl) {
        ...
        var listItems = Util.map(data.items, function (item) {
            ...
            if (item._abstract && item._abstract.length) {
                content = Util.map(item._abstract, function (line) {
                    ...
                    var rowClass = "div.inner-row." + (line.title ? "with-title" : "") + (line["class"] ? "." + line["class"] : "");
                    return m(rowClass, [line.title ? m("div.column.title", line.title) : null, m("div.column", line.text)]);
                });
            ...
            if (content === null) {
                content = Util.map(data.columns, function (col) {
                    ...
                    return m("div.inner-row.with-title", [m("div.column.title", col.title), m("div.column", colContent)]);
                });
            }
        ...
        return m("div.mobile", [m("div.row.mobile-header", [m("div.col-sm-6", [m("button.btn.btn-info.btn-block.toggle-btn", {
            onclick: function onclick() {
                ctrl.vm.showMobileFilterSettings(true);
            }
        }, [m("i.fa.fa-filter")], "Show filters")]), m("div.col-sm-6", [m("div.mobile-sort", getMobileSortSelect(ctrl))])]), ctrl.vm.showMobileFilterSettings() ? getMobileFilterModal(ctrl) : null, m("hr"), m("div.mobile-items", listItems), paginator(data.pagination, ctrl.setPage)]);
    }
    ...
    function renderMassActions(ctrl) {
        ...
        var totalItemCount = ctrl.vm.data().pagination.nItems;
        if (totalItemCount === 0) {
            return "";
        }
        var listedItemCount = ctrl.vm.data().items.length;
        var initialMassActions = [{ key: 0, value: gettext("Select Action") }, { key: "unselect_all", value: gettext("Clear Selections") }, { key: "select_all", value: gettext("Select All") }, { key: "select_listed", value: interpolate(gettext("Select All %s Items"), [listedItemCount]) }];
        massActions = initialMassActions.concat(massActions);

        return m("div.picotable-mass-actions", [ctrl.vm.allItemsSelected() ? m("p", interpolate(gettext("All %s Items Selected"), [totalItemCount])) : null, m("select.picotable-mass-action-select.form-control", {
    ...
    function renderHeader(ctrl) {
        ...
        return m("div.picotable-header", [renderMassActions(ctrl), m("div.picotable-items-per-page-ctr", [m("label", { "for": "pipps" + ctrl.id }, lang.ITEMS_PER_PAGE), m("select.picotable-items-per-page-select.form-control", {
            id: "pipps" + ctrl.id,
        ...
        }))]), m("div.picotable-item-info", itemInfo), m("div.picotable-reset-filters-ctr", m("button.picotable-reset-filters-btn.btn.btn-gray.btn-inverse", {
            onclick: ctrl.resetFilters,
            disabled: Util.isEmpty(ctrl.vm.filterValues())
        }, lang.RESET_FILTERS))]);
    }
    ...
    function PicotableView(ctrl) {
        return m("div.table-view", [ctrl.vm.showHeader() ? renderHeader(ctrl) : null, ctrl.vm.renderMode() === "mobile" ? renderMobileTable(ctrl) : renderTable(ctrl)]);
    }
    ...
    function PicotableController() {
        ...
        ctrl.setSortColumn = function (colId) {
            ...
            if (colId && colId.length && colId !== "null") {
                if (/^[+-]/.test(colId)) {
                    ...
                    var currentSort = ctrl.vm.sort();
                    if (currentSort === "+" + colId) sortValue = "-" + colId;else if (currentSort === "-" + colId) sortValue = null;else sortValue = "+" + colId;
                }
            }
        ...
        ctrl.saveCheck = function (object) {
            ...
            var items = originalValues.filter(function (i) {
                return i !== object._id;
            });
        ...
        ctrl.isChecked = function (object) {
            var originalValues = ctrl.vm.checkboxes();
            var checked = originalValues.filter(function (i) {
                return i === object._id;
            });
        ...
        ctrl.selectAllListedProducts = function () {
            ctrl.vm.allItemsSelected(false);
            ctrl.vm.checkboxes(ctrl.vm.data().items.map(function (item) {
                return item._id;
            }));
        };
        ...
        ctrl.refresh = function () {
            ...
            var params = m.route.parseQueryString(decodeURI(location.search));
    ...
    var generator = function generator(container, dataSourceUrl) {
...
/* eslint-disable */
if (typeof module !== "undefined" && module !== null && module.exports) module.exports = Picotable;else if (typeof define === "function" && define.amd) define(function () {
    return Picotable;
});
